...
#' @param init_age
#' integer between 0 and 110 denoting age of individual. This has to be the same initial
#' age used in the calculation of transition probability matrices.
#' @param closure_age
#' maximum life span
#' @param target_age
#' integer between 0 and 110 denoting the target age to survive to.
#' @param trans_probs
...
#'
#' @examples example
health3_surv_prob <- function(init_state, init_age, closure_age, target_age, trans_probs, end_state = NULL) {
...
#'
#' @examples example
health3_prob_plots <- function(init_age, closure_age, init_state, trans_probs) {
  ...
  for (target in (init_age+1):closure_age) {
    alive_probs <- append(alive_probs, health3_surv_prob(init_state, init_age, closure_age, target, trans_probs))
    healthy_probs <- append(healthy_probs, health3_surv_prob(init_state, init_age, closure_age, target, trans_probs, end_state = 0))
    disabled_probs <- append(disabled_probs, health3_surv_prob(init_state, init_age, closure_age, target, trans_probs, end_state = 1))
  }
