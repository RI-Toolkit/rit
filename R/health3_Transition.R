...
#' max age of closure age for 3 state model at a specified frequency.
#'
#' @param model_type
...
health3_get_trans_probs <- function(model_type, param_file, init_age, closure_age, female, year, freq) {
    ...
    # Pre-allocate numeric vectors for performance
    trans1 <- numeric(n_steps)
    trans2 <- numeric(n_steps)
    trans3 <- numeric(n_steps)
    trans4 <- numeric(n_steps)

    # integrate across each fractional year interval to get piecewise transition rates
    for (i in seq_len(n_steps)) {
        age_current <- ages[i]

        # transition 1 rates
        integral1 <- stats::integrate(cox_model, age_current, age_current + step, init_age = init_age, female = female,
                                      year = year, model_type = model_type, b = b1, gamma_age = gamma_age1,
                                      gamma_gender = gamma_gender1, gamma_time = gamma_time1, a = a1, v = v)
        trans1[i] <- integral1$value

        # transition 2 rates
        integral2 <- stats::integrate(cox_model, age_current, age_current + step, init_age = init_age, female = female,
                                      year = year, model_type = model_type, b = b2, gamma_age = gamma_age2,
                                      gamma_gender = gamma_gender2, gamma_time = gamma_time2, a = a2, v = v)
        trans2[i] <- integral2$value

        # transition 3 rates
        integral3 <- stats::integrate(cox_model, age_current, age_current + step, init_age = init_age, female = female,
                                      year = year, model_type = model_type, b = b3, gamma_age = gamma_age3,
                                      gamma_gender = gamma_gender3, gamma_time = gamma_time3, a = a3, v = v)
        trans3[i] <- integral3$value

        # transition 4 rates
        integral4 <- stats::integrate(cox_model, age_current, age_current + step, init_age = init_age, female = female,
                                      year = year, model_type = model_type, b = b4, gamma_age = gamma_age4,
                                      gamma_gender = gamma_gender4, gamma_time = gamma_time4, a = a4, v = v)
        trans4[i] <- integral4$value
    }
